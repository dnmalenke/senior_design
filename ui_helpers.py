...
def draw_path(frame, start_x, start_y, start_yaw, end_x, end_y, end_yaw, vectors, curvature, id, selected_arrow, selected_point):
    """
    Draws a path from start point to end point based on the provided vectors  
    id, selected_arrow, and selected_point are used for highlighting selected points/arrows  
    This function supports any order of 'L' 'R' and 'S' vectors  
    This function uses lines and ellipses to draw the path and does not interpolate a list of points for the path
    """
...
def draw_buttons(frame, running):
    """
    Draws start and stop buttons on the frame    
    """
...
def draw_tag(id, corners, frame, selected_box):
    """
    Draws a rectangle around a tag and information about its current position
    """
    x1,y1 = corners[0]
    x2,y2 = corners[1]
    x3,y3 = corners[2]
